...
public class CompetidorTableModelAgachamento extends AbstractTableModel{
    private Vector<String> nomeColunas = new Vector<String>();
    @SuppressWarnings("unchecked")
	private Vector<Vector> dados = new Vector<Vector>();

    public CompetidorTableModelAgachamento() {
        gerarNomeColunas();
    }

    public CompetidorTableModelAgachamento(List<Competidor> lista) {
        gerarNomeColunas();
        gerarDados(lista);
    }
	...
	private void gerarDados(List<Competidor> lista){
        for (Competidor competidor : lista){
            ...
            linha.add(competidor.getModalidades().getAgachamentoMelhorMarca());
            linha.add(competidor.getTentativas().getAgachamentoTentativa1());
            linha.add(competidor.getTentativas().getAgachamentoTentativa2());
            linha.add(competidor.getTentativas().getAgachamentoTentativa3());
